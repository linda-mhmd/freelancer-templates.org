/**
 * Property-Based Tests for Showcase Presets
 *
 * Uses fast-check to verify invariants for showcase preset slide configurations,
...
/** All showcase preset names */
const SHOWCASE_PRESET_NAMES = [
...
/** All showcase preset slide arrays for testing */
const ALL_SHOWCASE_SLIDES: Record<string, SlideConfig[]> = {
...
/**
 * Collect all slides from all showcase presets into a flat array with metadata
 */
function getAllShowcaseSlides(): Array<{
  presetName: string;
  slideIndex: number;
  slide: SlideConfig;
}> {
  ...
  for (const [presetName, slides] of Object.entries(ALL_SHOWCASE_SLIDES)) {
...
/**
 * Collect all slides with click targets from all showcase presets
 */
...
/**
 * Collect all slides with zoom regions from all showcase presets
 */
...
/**
 * Generate a valid showcase preset name
 */
const showcasePresetNameArb = fc.constantFrom(...SHOWCASE_PRESET_NAMES);
...
/**
 * Generate a random showcase slide by selecting a preset and index
 */
const showcaseSlideArb = fc.tuple(showcasePresetNameArb, slideIndexArb).map(([presetName, index]) => {
  const slides = ALL_SHOWCASE_SLIDES[presetName];
...
describe('Property 6: Screenshot Path Validity', () => {
  it('all showcase preset slides have non-empty screenshot paths for 100+ inputs', () => {
    const allSlides = getAllShowcaseSlides();
    ...
    fc.assert(
      fc.property(
        ...
        (slideIndex) => {
          ...
          // Screenshot path should start with 'tutorials/showcase/'
          expect(slide.screenshot.startsWith('tutorials/showcase/')).toBe(true);
  ...
  it('each showcase preset has exactly 5 slides with valid screenshot paths', () => {
    for (const presetName of SHOWCASE_PRESET_NAMES) {
      const slides = ALL_SHOWCASE_SLIDES[presetName];
  ...
  it('SHOWCASE_PRESETS object contains all 4 showcase presets', () => {
    expect(Object.keys(SHOWCASE_PRESETS).length).toBe(4);

    for (const presetName of SHOWCASE_PRESET_NAMES) {
      expect(SHOWCASE_PRESETS[presetName]).toBeDefined();
      expect(Array.isArray(SHOWCASE_PRESETS[presetName])).toBe(true);
    }
  });
});
...
describe('Property 7: Click Target Coordinate Bounds', () => {
  ...
  it('every showcase slide has a click target defined', () => {
    const allSlides = getAllShowcaseSlides();
...
describe('Combined Showcase Preset Validation', () => {
  it('all showcase presets pass all validation properties', () => {
    for (const presetName of SHOWCASE_PRESET_NAMES) {
      const slides = ALL_SHOWCASE_SLIDES[presetName];
  ...
  it('random sampling of slides maintains all invariants for 100+ inputs', () => {
    fc.assert(
      fc.property(showcaseSlideArb, ({ presetName, slideIndex, slide }) => {
        // Property 6: Screenshot path validity
        expect(typeof slide.screenshot).toBe('string');
        expect(slide.screenshot.length).toBeGreaterThan(0);
