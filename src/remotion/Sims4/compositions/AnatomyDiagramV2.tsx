// src/remotion/Sims4/compositions/AnatomyDiagramV2.tsx
// "Anatomy of an Agent" V2 — Vibrant blue gradient, servo bot left, flow diagram right
// Matches the NotebookLM reference with curved connecting lines and spatial layout
...
// Plumbob available from '../components/SimsUI' if needed

// ── Types ────────────────────────────────────────────────────────────────────
export interface AnatomyDiagramV2Props {
  diagramTitle?: string;
}
...
const LegendCard: React.FC<{
  ...
  >
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
      <span style={{ fontFamily: SIMS_FONTS.display, fontSize: 16, fontWeight: 800, color: SIMS_COLORS.textLight, textShadow: '0 2px 8px rgba(0,0,0,0.4)' }}>
        The {title}
      </span>
      <span style={{ fontFamily: SIMS_FONTS.body, fontSize: 11, color: 'rgba(255,255,255,0.75)', lineHeight: 1.35, maxWidth: 195, textShadow: '0 1px 4px rgba(0,0,0,0.3)' }}>
        {description}
      </span>
    </div>
  </div>
...
// ── Main Composition ─────────────────────────────────────────────────────────
export const AnatomyDiagramV2: React.FC<AnatomyDiagramV2Props> = ({
  ...
  // ── Layout coordinates (1280x720 canvas, left 380px = legend, right 900px = diagram)
  // All positions are ABSOLUTE within the 1280x720 frame
  // Right panel starts at x=380
  // User Query: center ~(530, 80)
  // Model hex: center ~(830, 90)
  // Orchestration: top-left ~(480, 230), size ~380x280
  // Tools: top-left ~(880, 230), size ~200x280
  // agent_response: center ~(700, 590)
  // User Output: center ~(1050, 590)
  ...
  return (
    ...
    >
      ...
      {/* ══════════════════════════════════════════════════════════════════
          ...
          Bot at top, then 3 glass panels below with arrows
          ══════════════════════════════════════════════════════════════════ */}
      <div
        style={{
          position: 'absolute',
      ...
      {/* ══════════════════════════════════════════════════════════════════
          RIGHT SIDE — Flow Diagram (absolute positioned elements)
          Coordinates are relative to the full 1280x720 canvas
          ══════════════════════════════════════════════════════════════════ */}
      ...
      >
        ...
        <path
          d="M 580 95 C 640 95, 720 75, 790 75"
          fill="none"
          stroke={FLOW_CYAN}
          strokeWidth="2.5"
          strokeLinecap="round"
        ...
        />
        <polygon points="790,70 802,75 790,80" fill={FLOW_CYAN} opacity={lineProgress} />

        {/* Cyan line: Model → Orchestration (down-left) */}
        <path
          d="M 830 155 C 830 190, 700 210, 660 240"
          fill="none"
          stroke={FLOW_CYAN}
          strokeWidth="2.5"
          strokeLinecap="round"
          ...
          style={{ filter: `drop-shadow(0 0 4px ${FLOW_CYAN})` }}
        />
        <polygon points="655,237 660,250 665,237" fill={FLOW_CYAN} opacity={lineProgress} />

        {/* Cyan line: Model → Tools (down-right) */}
        <path
          d="M 870 155 C 900 190, 1000 210, 1020 240"
          fill="none"
          stroke={FLOW_CYAN}
          strokeWidth="2.5"
          strokeLinecap="round"
          ...
          style={{ filter: `drop-shadow(0 0 4px ${FLOW_CYAN})` }}
        />
        <polygon points="1015,237 1020,250 1025,237" fill={FLOW_CYAN} opacity={lineProgress} />

        {/* Green line: Orchestration bottom → agent_response */}
        <path
          d="M 660 540 C 660 570, 750 590, 820 590"
          fill="none"
        ...
        />
        <polygon points="820,585 832,590 820,595" fill={ORCH_GREEN_LINE} opacity={lineProgress} />

        {/* Cyan line: agent_response → User Output */}
        <path
          d="M 960 590 C 1000 590, 1040 590, 1070 590"
          fill="none"
          stroke={FLOW_CYAN}
          strokeWidth="2.5"
          strokeLinecap="round"
          ...
          style={{ filter: `drop-shadow(0 0 4px ${FLOW_CYAN})` }}
        />
        <polygon points="1070,585 1082,590 1070,595" fill={FLOW_CYAN} opacity={lineProgress} />
      </svg>

      {/* ── User Query (top, between legend and model) ──────────────────── */}
      <div
        style={{
          position: 'absolute',
          left: 460,
      ...
      >
        <PersonIcon size={50} />
        <span style={{ fontFamily: SIMS_FONTS.display, fontSize: 15, fontWeight: 800, color: SIMS_COLORS.textLight, textShadow: '0 2px 8px rgba(0,0,0,0.4)' }}>
          User Query
        </span>
      </div>

      {/* ── Model Hexagon (top-right area) ──────────────────────────────── */}
      <div
        style={{
          position: 'absolute',
          left: 780,
      ...
      >
        ...
        <div style={{ textAlign: 'center', marginTop: 4 }}>
          <div style={{ fontFamily: SIMS_FONTS.display, fontSize: 17, fontWeight: 800, color: SIMS_COLORS.textLight, textShadow: '0 2px 8px rgba(0,0,0,0.4)' }}>
            Model
          </div>
          <div style={{ fontFamily: SIMS_FONTS.body, fontSize: 11, color: 'rgba(255,255,255,0.7)', maxWidth: 170, lineHeight: 1.3, marginTop: 2 }}>
            The centralized decision maker (LLM). Can be general purpose or fine-tuned.
          </div>
        </div>
      </div>

      {/* ── Orchestration Panel (green, large, center) ──────────────────── */}
      <div
        style={{
          position: 'absolute',
          left: 470,
      ...
      >
        ...
        >
          ...
          <div style={{ fontFamily: SIMS_FONTS.body, fontSize: 11, color: 'rgba(255,255,255,0.75)', marginBottom: 14, lineHeight: 1.35 }}>
          ...
          <div style={{ display: 'flex', gap: 10, marginBottom: 10 }}>
            ...
            >
              <div style={{ fontFamily: SIMS_FONTS.display, fontSize: 13, fontWeight: 700, color: SIMS_COLORS.textLight, marginBottom: 2 }}>
                Profile, goals,
              </div>
              <div style={{ fontFamily: SIMS_FONTS.display, fontSize: 13, fontWeight: 700, color: SIMS_COLORS.textLight }}>
                &amp; instructions
              </div>
            </div>
            ...
            >
              <div style={{ fontFamily: SIMS_FONTS.display, fontSize: 13, fontWeight: 700, color: SIMS_COLORS.textLight, marginBottom: 8 }}>
                Memory
              </div>
          ...
          >
            <span style={{ fontFamily: SIMS_FONTS.display, fontSize: 14, fontWeight: 700, color: SIMS_COLORS.textLight }}>
      ...
      {/* ── Tools Panel (gold, right of orchestration) ──────────────────── */}
      <div
        style={{
          position: 'absolute',
          left: 910,
      ...
      >
        ...
        >
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6 }}>
            ...
            <span style={{ fontFamily: SIMS_FONTS.display, fontSize: 17, fontWeight: 800, color: SIMS_COLORS.textLight, textShadow: '0 2px 6px rgba(0,0,0,0.3)' }}>
              Tools
            </span>
          </div>
          <div style={{ fontFamily: SIMS_FONTS.body, fontSize: 10, color: 'rgba(255,255,255,0.75)', marginBottom: 12, lineHeight: 1.35 }}>
            ...
            {toolItems.map((item, i) => {
              ...
              return (
                ...
                >
                  ...
                  <span style={{ fontFamily: SIMS_FONTS.display, fontSize: 12, fontWeight: 600, color: SIMS_COLORS.textLight }}>
                    {item}
                  </span>
                </div>
              );
      ...
      {/* ── agent_response (bottom center-right) ────────────────────────── */}
      <div
        style={{
          position: 'absolute',
          left: 835,
          ...
          transform: `translateX(${interpolate(sp(T_RESPONSE), [0, 1], [-20, 0], { extrapolateRight: 'clamp' })}px)`,
          zIndex: 10,
        }}
      >
        <span style={{ fontFamily: SIMS_FONTS.mono, fontSize: 14, fontWeight: 700, color: SIMS_COLORS.textLight, textShadow: '0 2px 6px rgba(0,0,0,0.4)' }}>
          agent_response
        </span>
      </div>
      ...
      <div
        style={{
          ...
          transform: `translateX(${interpolate(sp(T_OUTPUT), [0, 1], [20, 0], { extrapolateRight: 'clamp' })}px)`,
          zIndex: 10,
        }}
      >
        <span style={{ fontFamily: SIMS_FONTS.display, fontSize: 14, fontWeight: 800, color: SIMS_COLORS.textLight, textShadow: '0 2px 6px rgba(0,0,0,0.4)' }}>
          User Output
        </span>
        <PersonIcon size={46} />
      </div>
      ...
      >
        <span style={{ fontFamily: SIMS_FONTS.body, fontSize: 9, color: 'rgba(255,255,255,0.4)' }}>
