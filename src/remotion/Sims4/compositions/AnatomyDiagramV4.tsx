// src/remotion/Sims4/compositions/AnatomyDiagramV4.tsx
// "Anatomy of an Agent" V4 — Refined lines, positions, and layout from V3
// Changes from V3:
//   - Bot restored to ~top:145, legend cards at ~top:310 (20px lower than V3 cards)
//   - Legend cards width 330 (10px narrower than V3's 340)
//   - Tools panel moved further right (left:990)
//   - agent_response centered below Orch, User Output centered below Tools
//   - All SVG lines properly connected, exiting hex edges not through text
//   - Orch→agent_response arrow is a proper visible curved line
...
// ── Types ────────────────────────────────────────────────────────────────────
export interface AnatomyDiagramV4Props {
  diagramTitle?: string;
}
...
// ── Layout constants (V4 positions) ──────────────────────────────────────────
const LEFT_X = 30;
const LEFT_WIDTH = 360;
const BOT_TOP = 145;
const LEGEND_TOP = 310;
const LEGEND_CARD_WIDTH = 330;

const MODEL_LEFT = 780;
const MODEL_TOP = 63;
const MODEL_HEX_W = 116;
const MODEL_HEX_H = 116;
// MODEL center X = 838, bottom Y = 169 (used in SVG line calculations)

const ORCH_LEFT = 470;
const ORCH_TOP = 290;
const ORCH_WIDTH = 425;
// Computed centers: ORCH_CX ≈ 682.5, TOOLS_CX ≈ 1097.5, ORCH_BOTTOM ≈ 575

const TOOLS_LEFT = 990;
const TOOLS_TOP = 290;
const TOOLS_WIDTH = 215;

const RESPONSE_TOP = 620;
const OUTPUT_TOP = 620;
...
// ── Legend Panel (left side glass cards) ─────────────────────────────────────
const LegendCard: React.FC<{
  ...
  >
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
      ...
      <span style={{ fontFamily: SIMS_FONTS.body, fontSize: 13, color: 'rgba(255,255,255,0.75)', lineHeight: 1.35, maxWidth: 220, textShadow: '0 1px 4px rgba(0,0,0,0.3)' }}>
        {description}
      </span>
    </div>
  </div>
...
// ── Main Composition ─────────────────────────────────────────────────────────
export const AnatomyDiagramV4: React.FC<AnatomyDiagramV4Props> = ({
  ...
  return (
    ...
    >
      ...
      >
        {/* 1. User Query → Model hex: curve from person icon area to left edge of hex */}
        <path
          d="M 590 125 C 650 125, 720 110, 780 110"
          fill="none"
          stroke={FLOW_CYAN}
          strokeWidth="2.5"
          strokeLinecap="round"
        ...
        />
        <polygon points="775,105 788,110 775,115" fill={FLOW_CYAN} opacity={lineProgress} />

        {/* 2. Model → Orchestration: from hex bottom-left edge, wide curve to Orch top */}
        {/* Hex left-bottom edge point ≈ (788, 149). Orch top-center = (682, 290) */}
        <path
          d="M 788 152 C 760 200, 700 240, 682 285"
          fill="none"
          stroke={FLOW_CYAN}
          strokeWidth="2.5"
          strokeLinecap="round"
          ...
          style={{ filter: `drop-shadow(0 0 4px ${FLOW_CYAN})` }}
        />
        <polygon points="677,280 682,293 687,280" fill={FLOW_CYAN} opacity={lineProgress} />

        {/* 3. Model → Tools: from hex bottom-right edge, wide curve to Tools top */}
        {/* Hex right-bottom edge point ≈ (888, 149). Tools top-center = (1097, 290) */}
        <path
          d="M 888 152 C 920 200, 1060 240, 1097 285"
          fill="none"
          stroke={FLOW_CYAN}
          strokeWidth="2.5"
          strokeLinecap="round"
          ...
          style={{ filter: `drop-shadow(0 0 4px ${FLOW_CYAN})` }}
        />
        <polygon points="1092,280 1097,293 1102,280" fill={FLOW_CYAN} opacity={lineProgress} />

        {/* 4. Orchestration ↔ Tools: dashed horizontal between right edge of Orch and left edge of Tools */}
        <path
          d="M 895 410 L 990 410"
          fill="none"
        ...
        />
        <polygon points="985,406 995,410 985,414" fill="rgba(255,255,255,0.5)" opacity={lineProgress} />
        <polygon points="900,406 890,410 900,414" fill="rgba(255,255,255,0.5)" opacity={lineProgress} />

        {/* 5. Orchestration bottom → agent_response: green vertical drop */}
        {/* Orch panel bottom ≈ y:585, agent_response text at y:620 */}
        <path
          d="M 682 585 L 682 615"
          fill="none"
        ...
        />
        <polygon points="677,610 682,623 687,610" fill={ORCH_GREEN_LINE} opacity={lineProgress} />

        {/* 6. agent_response → User Output: horizontal cyan line */}
        {/* agent_response right edge ≈ x:755, User Output left edge ≈ x:990 */}
        <path
          d="M 755 630 C 830 630, 930 630, 990 630"
          fill="none"
          stroke={FLOW_CYAN}
          strokeWidth="2.5"
          strokeLinecap="round"
          ...
          style={{ filter: `drop-shadow(0 0 4px ${FLOW_CYAN})` }}
        />
        <polygon points="985,625 998,630 985,635" fill={FLOW_CYAN} opacity={lineProgress} />
      </svg>
      ...
      >
        <div style={{ position: 'relative', width: MODEL_HEX_W, height: MODEL_HEX_H }}>
          <svg width={MODEL_HEX_W} height={MODEL_HEX_H} viewBox="0 0 116 116" style={{ position: 'absolute', inset: 0 }}>
            <polygon
      ...
      {/* ── agent_response (centered below Orchestration) ─────────────── */}
      <div
        style={{
          position: 'absolute',
          left: ORCH_LEFT,
      ...
      {/* ── User Output (centered below Tools) ───────────────────────── */}
      <div
        style={{
          position: 'absolute',
          left: TOOLS_LEFT,
